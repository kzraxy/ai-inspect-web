<template>
  <div class="compliance_draw_rules_area">
    ...
    <div class="content_area">
      ...
      <div class="content_right">
        <div class="content_right_head"><div>区域绘制</div><div class="des">请在画面中绘制非上钟区域</div></div>
        <div class="draw_wrap">
          <div class="draw_area">
            <div class="content_right_draw">
              <div class="draw_tools_wrap">
                <div id="complianceDrawTools" class="draw_tools">
                  <drawTools ref="complianceDrawTools" :width='drawToolsW' :height='drawToolsH' :imgUrl='imgUrl' :supportPolygonType="rulesDrawData.supportPolygonType"
          ...
          <div class="draw_alone_btns">
            <div class="draw_alone_btns_wrap" :style="{'width':drawToolsW + 'px'}">
...
import { polygonRules, getCurrentDrawInfo, getNewPic, saveDrawImg, publishPolygonRules, returnNewPic } from '../proxy'
export default {
  components: {
    drawTools
  },
    ...
    refreshDrawImgFlag: {
      handler (val) {
        if (val) { // 当前通道正在抓图
          ...
          // this.imgUrl = 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1574239175006&di=c743fb3c0cad6d7e0a25beb2ac04e8db&imgtype=0&src=http%3A%2F%2Fi1.sinaimg.cn%2Fent%2Fd%2F2008-06-04%2FU105P28T3D2048907F326DT20080604225106.jpg'
        } else { // 当前通道停止抓图,清除定时器
  ...
  created () {
    ...
    this.extraTaskId = this.$route.params.extraTaskId || ''
  },
  mounted () {
    this.maxRefreshTimes = 0
    this.getChannelsList()// 获取规则、通道、预置点的数据
    let parentWidth = document.getElementById('complianceDrawTools').offsetWidth
    let parentHeight = document.getElementById('complianceDrawTools').offsetHeight
    ...
    window.onresize = () => {
      this.$refs.complianceDrawTools.getDrawInfo().then(res => {
        let parentWidth = document.getElementById('complianceDrawTools').offsetWidth
        let parentHeight = document.getElementById('complianceDrawTools').offsetHeight
        let value = res || []
        let polygon = []
        ...
        if (parentWidth / parentHeight > 960 / 576) {
          drawToolsH = document.getElementById('complianceDrawTools').offsetHeight - 30
  ...
  methods: {
    ...
    async publishPolygonRules () {
      ...
      this.$confirm('下发后刷新页面获取下发状态。请确认是否下发规则区域/线?', {
        ...
        publishPolygonRules({ extraTaskId: this.extraTaskId }).then(res => {
          if (+res.code === 0) {
    ...
    // 异步抓图，先请求抓图接口
    getNewPic () {
      getNewPic({ extraTaskId: this.extraTaskId, channelId: this.rulesDrawData.channelId }).then(pres => {
        if (+pres.code === 0) {
    ...
    returnNewPic () {
      ...
      returnNewPic({ extraTaskId: this.extraTaskId, channelId: this.rulesDrawData.channelId }).then(res => {
        if (+res.code === 0) {
          let data = res.data
          if (data.indexOf('http') > -1 || data.indexOf('https') > -1) {
            this.refreshDrawImgFlag = false
            this.$refs.complianceDrawTools.getDrawInfo().then(drawres => { // 将默认图上的绘制信息赋给新图
    ...
    async saveDrawImg (flag, item, index) {
      ...
      if (this.saveValid) {
        await saveDrawImg({ polygon: this.rulesDrawData.polygon, extraTaskId: this.extraTaskId, channelId: this.rulesDrawData.channelId, areaTypeId: 'NO_SERVICE_AREA' }).then(res => {
          if (+res.code === 0) {
    ...
    async getCurrentPolygonInfo () {
      await this.$refs.complianceDrawTools.getDrawInfo().then(res => {
        this.saveValid = true
    ...
    getChannelsList () {
      ...
      params.extraTaskId = this.extraTaskId
      params.areaKey = 'NO_SERVICE_AREA'
      polygonRules(params).then((res) => {
        if (res.code === 0) {
        // isPublished:1下发成功，2未下发，3下发失败
          let data = res.data.rows || []
          data.forEach(item => { // 复用之前的逻辑，一些值后端没有返回，前端默认赋值
            item.taskRuleId = null
            item.taskRuleName = null
            item.isPreset = null
    ...
    getRulesDrawData (data, index) { // data为当前正在编辑的通道信息
      ...
      params.areaTypeId = 'NO_SERVICE_AREA'
      getCurrentDrawInfo(params).then((res) => {
        if (res.code === 0) {
          let info = res.data
          this.rulesDrawData = { ...info }
          this.rulesDrawData.extraTaskId = this.extraTaskId

          // if (!info.isEdited) {
          if (!info.polygon || !info.polygon.length) {
