  ...
  <div class="area-select" ref="areaSelectRef">
    ...
    <transition name="fade">
      <div v-show="expand" class="expand-pane" :style="{'width': panelWidth+'px'}">
        <inputSearch @getClickData="getClickData" top="0" treeW="225" ref="inputSearch"></inputSearch>
      </div>
    </transition>
  </div>
</template>
...
export default {
  ...
  methods: {
    init(defaultGroupId, solutionTaskId) {
      this.$refs.inputSearch.init(defaultGroupId, solutionTaskId)
    },
    handlerMouseup (e) {
      this.$nextTick(() => {
