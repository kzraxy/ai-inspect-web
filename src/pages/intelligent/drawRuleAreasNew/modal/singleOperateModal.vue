<template>
  <el-dialog title="单个操作" :visible="visible" @close="close" :area="500" :close-on-click-model=false>
...
export default {
  ...
  methods: {
    getOnlyLabelList() {
      getOnlyLabelList({ ruleId: this.ruleId, id: this.currentLeftItem.id }).then(res => {
        if (res.code === 0) {
          this.areaLabelList = res.data || []
          this.form.connectType = this.currentLeftItem.connectType
          this.form.connectAreaLabelIds = this.ruleSearchMultiLabelSelect ? this.currentLeftItem.connectAreaLabelIds : this.currentLeftItem.connectAreaLabelIds[0]
        }
      })
    },
    save () {
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          this.$emit('singleFinish', this.form)
        }
      })
    },
    close () {
