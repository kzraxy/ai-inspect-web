        ...
        <el-scrollbar wrap-class="el-demo2-scrollbar__wrap" view-class="el-demo2-scrollbar__view" v-show="isSearch">
          <ul class="f-storeList" v-if="hackReset" @click.stop>
            <template v-if="!isMultiSearch">
              ...
              <li v-if="item.groupName" v-for="item in storeFilterList" @click="item.selectable && storeSelect(item)" :key='item.index' class="item-chose" v-bind:class="{'itemActive':item.isSelected, 'itemDisabled':!item.selectable}">{{item.groupName}}</li>
            </template>
            <template v-else>
              ...
              <template v-for="(block, idx) in storeFilterList">
                ...
                <li v-if="item.groupName" v-for="item in block" @click="item.selectable && storeSelect(item, idx)" :key='item.index' class="item-chose" v-bind:class="{'itemActive':item.isSelected, 'itemDisabled':!item.selectable}">{{item.groupName}}</li>
              </template>
            </template>
          </ul>
        </el-scrollbar>
...
export default {
  ...
  methods: {
        ...
        if (this.onlyCheckLeaf) {
          this.loadNode(node, res => {
            if (this.isStore) {
              ...
              if ((data.groupType === 1 || (res.length === 0 && data.groupType !== 0)) || this.serviceType === 'minerva') { // 普教非叶子节点可以返回事件
                resolve(true)
              } else {
      ...
      getTreeNext(this.treeAnsyProps).then(res => {
        if (res.code * 1 === 0 && res.data) {
          ...
          if (this.treeData.parentId === '' || !this.treeData.parentId || this.treeData.parentId === '-1') {
            ...
            this.$emit('getOriginArea', this.originArea.groupId)
          }
          // 默认选中第一条
    ...
    searchStoreList (prop) {
      ...
      if (prop.params && prop.params.groupName) {
        prop.params.groupName = this.nameLike
      } else {
        prop.params.nameLike = this.nameLike
