<template>
  <div class="tree-frame" v-bind:style="{top:top+'px',height:height+'px'}">
    <div class="tree-filter">
      <el-input v-if="isFilter" class="tree-filter-inp" :placeholder="placeHolder" suffix-icon="h-icon-search" :style="{width:treeW}"
      ...
      <div class="tree-wrap" :style="{width:treeW,bottom:treeBot+'px'}">
        <el-tree class="tree-left" v-show="!isSearch" :before-click="beforeClick" default-icon="" :data="treeData" simple-data parent-key="parentId"
        ...
        @node-click="getClickNode" :load="loadNode" lazy :expand-on-click-node="isExpandClickNode" @node-expand="hideTips">
        </el-tree>
        <!-- 搜索结构 -->
        <el-scrollbar wrap-class="el-demo2-scrollbar__wrap" view-class="el-demo2-scrollbar__view" v-show="isSearch">
          <ul class="f-storeList" v-if="hackReset" @click.stop>
            ...
            <!-- <li v-if="item.storeName" v-for="item in storeFilterList" @click="storeSelect(item, true)" :key='item.index' class="item-chose" v-bind:class="{'itemActive':item.isSelected}">{{item.storeName}}</li> -->
            ...
            <!-- <li v-if="item.groupName" v-for="item in storeFilterList" @click="storeSelect(item, true)" :key='item.index' class="item-chose" v-bind:class="{'itemActive':item.isSelected}">{{item.groupName}}</li> -->
...
export default {
  ...
  methods: {
    ...
    getTree () {
      ...
      // this.treeAnsyProps.params.groupId = ''
