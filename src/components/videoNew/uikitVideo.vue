...
<script>
import { getDeviceRamAccount, getTokenStrategy, getTkToken } from './proxy'
import { dataURLtoBlob } from '@/common/util'
import waterMark from '@/components/waterMark/simpleWaterMark'
...
export default {
  ...
  data() {
    return {
      ...
      downloadVideoModalVisible: false,
  ...
  created() {
    ...
    this.playerControllers = new Array(4).fill(1).map((e, idx) => this.genPlayerController(idx))
  },
  mounted() {
    this.getThemeData()
  ...
  destroyed() {
    // this.globalEventBus.$off('resize', this.resizeTrigger)
  ...
  computed: {
    ...
    playModelTag() {
      return ['live', 'rec'][this.playModel]
    },
    currentThemeData() {
      
      let td = this.themeData[['previewTheme', 'playBackTheme'][this.playModel]] || { header: {btnList: []}, footer: {btnList: []}}
  ...
  methods: {
    ...
    reportPlayInfo(playInfo) {
      // this.globalEventBus.$emit('uikit_select_channel_changed', playInfo)
    },
    resizeHandler() {
      this.syncSize()
      // 全屏情况下，不触发resize
    ...
    downloadClicked() {
      if (!this.currentPlayerController.playbackRecording) {
        this.downloadVideoModalVisible = true
      } else {
        this.$message.warning('当前有下载任务正在进行，请稍后再试')
      }
    },
     ...
     */
    async tokenModeInit() {
      const {code, data} = await getTokenStrategy()
      if (code===0) {
    ...
    async genTokenInfo(apiMap, apiParams) {
      return new Promise(async (resolve, reject) => {
        try {
          ...
          const res = await apiFunc(apiParams[this.tokenMode])
          resolve(res)
        } catch(e) {
          if (this.tokenMode===2) {
    ...
    capturePicture(info) {
      ...
      pc.player.capturePicture('default',async img => {
        ...
        clearTimeout(this.capturePictureTimer)
        this.capturePictureTimer = null
        let res = await Promise.all([this.uploadFile(img), pc.player.getOSDTime().catch()])
        let data = { ...pc.playInfo, picUrl: res[0], captureGuid: info.captureGuid }
        if (res[1] && res[1].code == 0) {
          data.playTime = res[1].data
        }
        this.showFeedbackReasonIfNeeded(pc.playInfo, 2).then(() => {
          callback(data)
        })
        // this.$emit('picCaptured', data)
      }, false).catch(e => {
    ...
    ///
    // { fileName: 'xxx.jpeg', base64: '' }, 升级8.0.5后fileUint8Array字段去除
    async uploadFile(imgObject) {
      try {
        let file = new File([dataURLtoBlob(imgObject.base64)], "default", {
          type: "image/jpeg",
        })
        const result = await this.$ossUploader.upload(file, "080101")
        return result.url
      } catch (e) {
    ...
    // 向外部业务更新当前的监控点
    updatePlayingMonitorList() {
      // this.globalEventBus.$emit('mac_playing_monitor_list', {
      //   fireTime: Date.now(),
      //   cameraList: this.playerControllers.slice(0, this.playerCount).flatMap(pc => pc.player ? pc.playInfo : []),
      // })
    },
    ...
    playVideo(pc) {
      ...
      let playUrl = pc.url
      if (this.playModel && pc.playInfo.startTime) {
      ...
      if (pc.player) {
        ...
        //   // this.resizeHandler()
        // }
        // pc.player.stop().then(playCallback).catch(playCallback)
      ...
      pc.player = new EZUIKit.EZUIKitPlayer({
        ...
        // startSaveCallBack(res) {
        // },
        stopSaveCallBack(res) {
          // 自定义按钮触发录制后，再通过内置按钮录制且下载时，同步录制状态
          pc.playbackRecording = false
        },
      })
...
<style lang="scss">
#uikit_0-speed, #uikit_1-speed, #uikit_2-speed, #uikit_3-speed {
  display: none;
}
</style>
