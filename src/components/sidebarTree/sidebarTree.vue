    ...
    <div class="treePart" v-loading="searching">
      ...
      <el-scrollbar v-if="filterFlag" wrapStyle="height:100%;overflow-x:hidden;">
        <ul class="fStoreList"  @click.stop>
          <span v-show='areaList.length === 0' class="msgTip">未查询到任何{{needStore ? applicationSceneName : '区域'}}！</span>
...
export default {
  ...
  watch: {},
  async mounted () {
    if (!['open'].includes(this.serviceType)) {
      this.initTree()
    }
  },
  methods: {
    async initTree () {
      try {
        const { code, data } = await getAreaTree()
        if (+code === 0 && data) {
          this.treeData = data || []
          if (this.treeData.length) {
            ...
            if (this.defaultSelect) {
              this.$nextTick(() => {
                this.$refs.sidebarTree && this.$refs.sidebarTree.setCurrentKey(this.treeData[0].nodeId)
