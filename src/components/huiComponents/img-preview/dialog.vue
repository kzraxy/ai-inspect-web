  ...
  <div :class="[`h-img-preview--theme-${$parent.theme}`]" class="h-img-preview">
    <transition name="zoom" @after-enter="afterEnter" @after-leave="afterLeave">
      <div
        ...
        @click.prevent="handleClickWrapper"
      >
...
// components
import HImgSnippetsBtnGroup from '@hui-pro/img-snippets/src/btn-group.vue'
import HImgCarousel from '../img-carousel/img-carousel.vue'
// import HImgCarousel from '@hui-pro/img-carousel/src/img-carousel.vue'
import HAlbum from '../album/album.vue'
// import HAlbum from '@hui-pro/album/src/album.vue'

export default {
  components: {
    HImgSnippetsBtnGroup,
    HImgCarousel,
    HAlbum
  },
  data () {
  ...
  methods: {
    $update () {
      const ImgCarouselCtor = Vue.extend(HImgCarousel)
      const AlbumCtor = Vue.extend(HAlbum)
    ...
    handleClickWrapper (e) {
      ...
      const currentRef = carouselRef.$getItemRef()
      const imgRef = currentRef ? currentRef.$refs.img : null
      if (
        currentRef &&
        imgRef &&
        e.target !== imgRef &&
        this.$parent.maskClosable &&
        this.$parent.data.length === 1
      ) {
        this.$close()
      }
    }
