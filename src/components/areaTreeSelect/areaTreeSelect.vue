  ...
  <div class="area-select" ref="areaSelectRef">
    ...
    <transition name="fade">
      <div v-show="expand" class="expand-pane" :style="{'width': panelWidth+'px'}">
        <TreeAnsySearch :treeAnsyProps=treeAnsyProps :storeListProps=storeListProps :isInitSelected=true  @getClickData="getClickData" top="0" treeW="225"></TreeAnsySearch>
      </div>
    </transition>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getClickData (area) {
      ...
      if ((this.isOnlyChooseLeaf && area.groupType === 0) && this.serviceType !== 'open' && this.serviceType !== 'minerva') { //开发者服务非子节点也可以挂设备
        return false
      }
