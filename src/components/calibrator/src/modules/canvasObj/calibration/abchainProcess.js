...
class ABChainProcess extends CalibrationProcess {
  constructor(params) {
    super(params);
    this.updateProcessData(this.scope.coordinates[0]);
  }
}
ABChainProcess.prototype.updateProcessData = function(point, limit) {
  let X = point.x;
  let Y = point.y;
  if (limit) {
...
ABChainProcess.prototype.enlarge = function({ newRatio, oldRatio, newPoint, oldPoint }) {
...
ABChainProcess.prototype.getScope = function() {
  const {
...
ABChainProcess.prototype.setScope = function(scope) {
...
ABChainProcess.prototype.unFormat = function(image, calibrationKey, indexKey) {
...
ABChainProcess.prototype.move = function(distX, distY) {
  this.moveControlsAndCoordinates(distX, distY);
};
export default ABChainProcess;
