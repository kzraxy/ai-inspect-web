...
class ABChain extends Calibration {
  constructor(data) {
    super(data);
  }
}

ABChain.prototype.getScope = function () {
  const {
...
ABChain.prototype.setScope = function (scope) {
...
ABChain.prototype.format = function (data) {
  const {
    type: propType,
...
ABChain.prototype.unFormat = function (image, calibrationKey, indexKey) {
...
ABChain.prototype.enlarge = function ({ newRatio, oldRatio, newPoint, oldPoint }) {
...
ABChain.prototype.move = function (distX, distY) {
  this.moveControlsAndCoordinates(distX, distY);
  this.updateLabelPoint();
};
ABChain.prototype.adjustControl = function (index, { distX, distY }) {
