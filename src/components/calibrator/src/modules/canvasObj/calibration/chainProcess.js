...
class ChainProcess extends CalibrationProcess {
  constructor(params) {
    super(params);
    this.updateProcessData(this.scope.coordinates[0]);
  }
}
ChainProcess.prototype.updateProcessData = function(point, limit) {
  let X = point.x;
  let Y = point.y;
  if (limit) {
...
ChainProcess.prototype.enlarge = function({ newRatio, oldRatio, newPoint, oldPoint }) {
...
ChainProcess.prototype.getScope = function() {
  const {
...
ChainProcess.prototype.setScope = function(scope) {
...
ChainProcess.prototype.unFormat = function(image, calibrationKey, indexKey) {
...
ChainProcess.prototype.move = function(distX, distY) {
