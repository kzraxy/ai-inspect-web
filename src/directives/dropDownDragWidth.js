import Vue from 'vue'
Vue.directive('dropDownDragWidth', {
  bind(el, binding, vnode, oldVnode) {
    const dragDomClass = binding.value?.className || ''
    // 定义最小宽度
    const minW = binding.value?.minWidth || 190;
    // 定义最大宽度
    const maxW = binding.value?.maxWidth || -1;
    // 最大宽度不能小于最小宽度
    if (maxW>-1 && maxW < minW) {
      return
    }
    ...
    let startWidth = 0;
    // 获取弹窗内容元素
    const dragDom = el.querySelector('.' + dragDomClass);
    if (!dragDom) return
    // 初始化是否正在调整大小的标志
